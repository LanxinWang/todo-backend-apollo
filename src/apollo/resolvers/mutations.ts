...
const mutations: MutationResolvers = {
  Mutation: {
    ...
    deleteAllCompletedTodos: (_, {deletedIds}, { dataSources }): Promise<Todo[]> => {
      return dataSources.todosAPI.deleteAllCompletedTodos(deletedIds);
    },

    updateATodoStatus: (_, { _id, isChecked }, { dataSources }): Promise<Todo> => {
      return dataSources.todosAPI.updateATodoStatus(_id, isChecked);
    },

    updateAllTodosStatus: (_, { updateIds, isChecked }, { dataSources }): Promise<Todo[]> => {
